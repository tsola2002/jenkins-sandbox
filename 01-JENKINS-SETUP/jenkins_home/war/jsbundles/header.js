...
function computeActions() {
  document.querySelectorAll(".jenkins-header__actions .jenkins-button[data-type='header-action'].jenkins-hidden").forEach(e => {
  ...
  const items = [];
  const actions = Array.from(document.querySelectorAll(".jenkins-header__actions .jenkins-button[data-type='header-action']")).slice(1, -1);
  ...
  utils.generateDropdown(overflowButton, instance => {
...
function generateOverflowButton() {
  ...
  const element = createElementFromHtml(`<button id="${OVERFLOW_ID}" class="jenkins-button jenkins-button--tertiary"><svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 512 512">
  ...
  actionsContainer.insertBefore(element, actionsContainer.children[actionsContainer.children.length - 2]);
  return element;
}
...
function computeBreadcrumbs() {
  ...
  if (!breadcrumbsBarOverflows()) {
    breadcrumbs_overflow_removeOverflowButton();
    return;
  }
  const items = [];
  const breadcrumbs = Array.from(document.querySelectorAll(`[data-type="breadcrumb-item"]`));
  const breadcrumbsOverflow = breadcrumbs_overflow_generateOverflowButton().querySelector("button");
...
function breadcrumbs_overflow_generateOverflowButton() {
  // If an overflow menu already exists let's use that
...
function breadcrumbs_overflow_removeOverflowButton() {
...
/******/ 	var __webpack_exports__ = __webpack_require__.O(undefined, [96], function() { return __webpack_require__(6817); })
